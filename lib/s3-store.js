...
module.exports.initialize = function(jsonConfigLocation, createFiles)
{
	var defer = Q.defer();

  	//grab our access info
  	var accessInfo = JSON.parse(fs.readFileSync(jsonConfigLocation));

	console.log('\t S3 Store Config Info:  '.blue, accessInfo);

	//load in our config info
	AWS.config.loadFromPath(jsonConfigLocation);
	...
	{
		console.log('Ready with redis');

		//create our s3 storage object
		var s3StoreObject = new S3ObjectClass(accessInfo, createFiles, redisClient);

		//grab it yo -- send it back
		defer.resolve({s3Store: s3StoreObject, redis: redisClient});
	});
